  ...
  const getCategoryBadge = (cat: string) => {
    switch (cat) {
      case 'cross_source_conflict':
        return (
          <span className="px-2 py-0.5 rounded bg-red-500/10 text-red-500 border border-red-500/20 text-[11px] font-mono font-semibold">
            ⚔️ Conflict
          </span>
        );
      case 'low_confidence':
      ...
      case 'missing_attribute':
        return (
          <span className="px-2 py-0.5 rounded bg-accent text-foreground border border-border text-[11px] font-mono font-semibold">
            ❓ Missing Field
          </span>
        );
      default:
    ...
    <div className="space-y-6 text-foreground">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-3.5 py-2 bg-card hover:bg-accent text-foreground text-xs font-semibold rounded-lg border border-border transition flex items-center gap-2"
        >
          <RefreshCw className={`w-3.5 h-3.5 text-muted-foreground ${loading ? 'animate-spin' : ''}`} /> Refresh Reviews
        </button>
      </div>

      {/* TOP KPI CARDS */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <div className="p-5 bg-card border border-border rounded-xl shadow-lg space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Total active review items</p>
        </div>
        ...
        <div className="p-5 bg-card border border-border rounded-xl shadow-lg space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Competing claim discrepancies</p>
        </div>
        ...
        <div className="p-5 bg-card border border-border rounded-xl shadow-lg space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Confidence below threshold</p>
        </div>
        ...
        <div className="p-5 bg-card border border-border rounded-xl shadow-lg space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Rule & format violations</p>
        </div>
        ...
        <div className="p-5 bg-card border border-border rounded-xl shadow-lg space-y-2">
          ...
          <div className="text-3xl font-black text-foreground font-medium">{summary?.products_needing_review ?? 0}</div>
          <p className="text-xs text-muted-foreground">Catalog products requiring action</p>
        </div>
      </div>

      {/* FILTER & SEARCH BAR */}
      <div className="bg-card border border-border rounded-xl p-4 shadow-lg flex flex-wrap items-center justify-between gap-4">
        <form onSubmit={handleSearchSubmit} className="flex items-center gap-2 flex-1 min-w-[240px]">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-background border border-border text-foreground pl-9 pr-4 py-2 rounded-lg text-xs font-medium outline-none focus:border-indigo-500 transition"
            />
          </div>
          <button
            type="submit"
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="bg-background border border-border text-foreground px-3 py-2 rounded-lg text-xs font-medium outline-none"
          >
      ...
      {isEmpty && !loading && !error && (
        <div className="p-12 border border-dashed border-emerald-900/60 bg-emerald-500/10/20 rounded-xl flex flex-col items-center justify-center text-center space-y-4 shadow-xl">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <button
              ...
              className="px-4 py-2 bg-accent hover:bg-card text-foreground border border-border rounded-lg text-xs font-semibold flex items-center gap-2 transition"
            >
      ...
      {!isEmpty && (
        <div className="bg-card border border-border rounded-xl p-6 shadow-xl space-y-4">
          ...
          {loading ? (
            ...
            <p className="text-xs text-muted-foreground italic py-8 text-center">No review items match your active filters.</p>
          ) : (
                    ...
                    >
                      <td className="py-3 px-3">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className="px-1.5 py-0.5 rounded bg-accent border border-border text-[10px] font-mono text-foreground">
                            {item.brand}
                          </span>
                      ...
                      <td className="py-3 px-3">
                        ...
                        <div className="text-[11px] font-mono text-muted-foreground">{item.attribute_name || '—'}</div>
                      </td>
                      ...
                      <td className="py-3 px-3 font-mono text-xs text-muted-foreground">{formatApiDateTime(item.created_at)}</td>
                      <td className="py-3 px-3 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 bg-foreground text-background hover:bg-transparent hover:text-foreground text-white rounded text-xs font-semibold transition"
                        >
                          Inspect & Resolve
                        </button>
                      </td>
      ...
      {selectedReview && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 text-foreground relative">
            {/* Close Button */}
            <button
              onClick={() => {
              ...
              className="absolute top-4 right-4 p-1.5 text-muted-foreground hover:text-white bg-accent hover:bg-card rounded-lg transition"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-2 border-b border-border pb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground font-mono">SKU: {selectedReview.sku}</span>
              ...
              <p className="text-xs text-muted-foreground font-mono">
                Attribute: <span className="text-foreground font-semibold">{selectedReview.display_name}</span> ({selectedReview.attribute_name || 'N/A'})
              </p>
            </div>

            {/* Success notification alert */}
            {resolutionSuccess && (
              <div className="p-4 bg-emerald-500/10/80 border border-emerald-500/20 rounded-lg text-emerald-500 text-xs font-semibold flex items-center justify-between">
            ...
            <div className="bg-background border border-border rounded-lg p-4 space-y-2">
              <div className="flex items-center justify-between text-xs text-muted-foreground font-mono">
                <span>Validation Rule: <strong className="text-foreground">{selectedReview.validation_type}</strong></span>
              ...
              <p className="text-sm text-foreground leading-relaxed font-medium">{selectedReview.message}</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-background border border-emerald-900/80 rounded-lg p-4 space-y-2">
                ...
                <div className="bg-background border border-amber-900/80 rounded-lg p-4 space-y-2">
                  ...
                  <p className="text-[11px] text-muted-foreground">Distributor / Cross-Attribute Check</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {selectedReview.evidence.length === 0 ? (
                <p className="text-xs text-muted-foreground italic p-4 bg-background rounded-lg text-center">
                  No evidence quotes attached to this attribute.
                </p>
              ) : (
                <div className="space-y-3 max-h-48 overflow-y-auto pr-1">
                  {selectedReview.evidence.map((ev) => (
                    <div key={ev.id} className="bg-background border border-border rounded-lg p-3 space-y-1.5 text-xs">
                      <div className="flex items-center justify-between text-muted-foreground font-mono text-[11px]">
                        <span className="inline-flex items-center gap-1.5">
                          <FileText className="w-3 h-3 text-muted-foreground" />
                          {ev.document_filename || ev.source_name} (Page {ev.page_number || 1})
                        </span>
            ...
            {selectedReview.status === 'open' && (
              <div className="pt-4 border-t border-border space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-xs text-muted-foreground font-mono block">Reviewer Notes (Optional):</label>
                  <input
                    type="text"
                    placeholder="Add audit resolution notes..."
                    value={resolutionNotes}
                    onChange={(e) => setResolutionNotes(e.target.value)}
                    className="w-full bg-background border border-border text-foreground px-3 py-2 rounded-lg text-xs font-medium outline-none"
                  />
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-accent hover:bg-card text-foreground border border-border rounded-lg text-xs font-semibold transition disabled:opacity-50"
                  >
                    Set Custom Value...
                  </button>
                </div>
                ...
                {showCustomInput && (
                  <div className="p-4 bg-background border border-border rounded-lg flex items-center gap-3">
                    <input
                      type="text"
                      placeholder="Enter verified custom value..."
                      value={customValueInput}
                      onChange={(e) => setCustomValueInput(e.target.value)}
                      className="flex-1 bg-card border border-border text-foreground px-3 py-2 rounded-lg text-xs font-medium outline-none"
                    />
                    <button
                      disabled={resolving || !customValueInput.trim()}
                      onClick={() => handleResolve(selectedReview, 'custom_value', customValueInput.trim())}
                      className="px-4 py-2 bg-foreground text-background hover:bg-transparent hover:text-foreground text-white rounded-lg text-xs font-semibold transition disabled:opacity-50"
                    >
                      Submit Custom Value
                    </button>
                  </div>
            ...
            {selectedReview.status === 'resolved' && (
              <div className="p-4 bg-emerald-500/10/60 border border-emerald-500/20 rounded-lg text-xs text-emerald-500 flex items-center justify-between font-mono">
                <span>✓ Resolved by {selectedReview.resolved_by || 'human_reviewer'} on {formatApiDateTime(selectedReview.resolved_at)}</span>
              </div>
            )}
          </div>
        </div>
