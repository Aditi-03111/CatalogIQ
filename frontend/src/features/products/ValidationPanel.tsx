...
export const ValidationPanel: React.FC<ValidationPanelProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Completeness</span>
          ...
          <div className="text-right bg-slate-800 px-4 py-2 rounded-lg border border-slate-700">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Quality Score</span>
      ...
      {/* Issues List */}
      {issues.length === 0 ? (
        <div className="bg-emerald-950/30 border border-emerald-800/50 rounded-lg p-4 text-emerald-300 flex items-center gap-3">
          ...
          {issues.map((issue, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-slate-500 font-mono">{issue.validation_type}</span>
              </div>

              <p className="text-sm text-slate-300">{issue.message}</p>

              {(issue.expected_value || issue.actual_value) && (
                <div className="grid grid-cols-2 gap-4 bg-slate-900/80 p-3 rounded border border-slate-800/80 text-xs font-mono">
                  <div>
                    <span className="text-slate-500 block">Expected Value:</span>
                    <span className="text-emerald-400">{String(issue.expected_value || 'N/A')}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Actual Extracted:</span>
              ...
              {issue.id && (
                <div className="flex items-center gap-2 pt-2 border-t border-slate-800/60">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded text-xs font-medium transition disabled:opacity-50"
                  >
                    Set Custom Value...
                  </button>
                </div>
