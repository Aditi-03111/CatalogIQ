...
export const Sidebar: React.FC = () => {
  return (
    <aside className="w-72 border-r border-border bg-background flex flex-col h-screen select-none rounded-none transition-colors duration-200">
      {/* Brand Header */}
      <Link 
        to="/" 
      ...
      <nav className="flex-1 mt-6 space-y-0.5 overflow-y-auto">
        ...
        <SidebarItem to="/jobs" label="Processing" icon={Activity} />
        <SidebarItem to="/search" label="Search" icon={Search} />
        <SidebarItem to="/reviews" label="Reviews" icon={CheckSquare} />
        <SidebarItem to="/unilog" label="Enrichment Report" icon={FileText} />
        <SidebarItem to="/health" label="Health" icon={HeartPulse} />
      </nav>
      ...
      <div className="border-t border-border flex flex-col">
        ...
        <SignOutButton redirectUrl="/login">
          <button
            ...
            className="group flex items-center gap-3 px-4 py-3 text-xs uppercase tracking-widest font-light transition-all duration-150 rounded-none text-muted-foreground hover:bg-card hover:text-foreground text-left"
          >
            <LogOut className="w-4 h-4 shrink-0 opacity-80 group-hover:opacity-100 transition-opacity" />
            <span>Sign Out</span>
          </button>
        </SignOutButton>
      </div>
